...
def databases(request):
  ...
  db = _get_db(user=request.user, cluster=cluster, access_token=spark_access_token)
...
def drop_database(request):
  ...
  cluster = json.loads(request.POST.get('cluster', '{}'))
  spark_access_token = {'SPARK_ACCESS_TOKEN': request.session.get('oidc_access_token')}
  db = _get_db(user=request.user, source_type=source_type, cluster=cluster, access_token=spark_access_token)
...
def _get_db(user, source_type=None, cluster=None, access_token: dict = None):
  ...
  query_server = get_query_server_config(name=name, connector=cluster, access_token=access_token)
